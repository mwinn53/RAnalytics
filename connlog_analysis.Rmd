...
```{r networkData, echo = FALSE, message = FALSE, warning = FALSE}
  ...
  ### Calculate additional descriptive variables    
    ...
    ex <- select(data, orig_bytes, resp_bytes, missed_bytes, total_bytes, orig_ip_bytes, resp_ip_bytes, total_ip_bytes, orig_pkts, resp_pkts, total_pkts)
    
    summarizeColumns(ex)[c(2,3,14,4,13,15,16)]
...
```{r summary, echo = FALSE, message = FALSE, warning = FALSE}
  ...
  g <- ggplot(data, aes(x=as.factor(vlan))) +
    geom_bar(stat = "count") +
    labs(x = "VLAN ID" , y = "Frequency") +
    ggtitle("Overall VLAN Density")
  print(g)
  
  g <- ggplot(data, aes(x=as.factor(service))) +
    geom_bar(stat = "count") +
...
```{r heatmap, echo = FALSE, message = FALSE, warning = FALSE}
  ...
  heatmap(m, col = topo.colors(max(length(unique(data$vlan)), length(unique(data$service)))), main = "Total Packets between VLANs and services", xlab = "service", ylab = "VLAN ID")
